...
| `LOG_DIRECTORY` | unset | LEGACY, superseded by `CLOUDE_STATE_DIR`. Only relevant to an install upgrading from before `CLOUDE_STATE_DIR` existed: if set, and if `CLOUDE_STATE_DIR` is NOT set, then `refresh_tokens.db` / `session_metadata.json` / `pinned_themes.json` / `unread_state.json` are both READ FROM and WRITTEN TO here whenever the new location does not have them yet. That is deliberate, so an install that started writing here keeps writing here instead of silently abandoning its data one directory over, and it means this IS a write target. Setting `CLOUDE_STATE_DIR` suppresses it entirely: a named state directory outranks this one, which is what makes that one variable enough to isolate a dev or test instance |

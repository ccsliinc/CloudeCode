...
| `CLAUDE_CLI_PATH`        | No                 | auto-detect      | Unused by session launch. To point the `claude` agent type at a plain binary, set `agents.claude_command` in `config.json` instead (see below) |
...
| Key                | Type | Default | Purpose                                                                                    |
| ------------------ | ---- | ------- | -------------------------------------------------------------------------------------------- |
| `claude_command`   | str  | `""`    | Shell command used to launch the `claude` agent type. Empty (default) falls back to the built-in `cld` / `cldor` zsh-function launcher (see "Launching Claude with a custom alias" below). Set this to a plain invocation, e.g. `"claude --dangerously-skip-permissions"` (see `config.example.json`), to run Claude Code directly on machines that don't define `cld`/`cldor`. Still wrapped in `zsh -c 'source ~/.zshrc >/dev/null 2>&1; <cmd>'` so both a plain binary and a shell function work. |
| `codex_command`    | str  | `"codex"` | Command for `agent_type == "codex"`                                                        |
| `hermes_command`   | str  | `"hermes"` | Command for `agent_type == "hermes"` (NOT `hermes-agent`)                                 |
| `openclaw_command` | str  | `"openclaw tui"` | Command for `agent_type == "openclaw"` (NOT bare `openclaw`)                          |
| `shell_command`    | str  | `"$SHELL -i"` | Plain interactive shell for the "New console" FAB action, no agent CLI                 |

`CLAUDE_CLI_PATH` (`.env`) remains unused for session launch regardless of
`claude_command` — it only affects `Settings.get_claude_cli_path()`, which
has no callers in the launch path.

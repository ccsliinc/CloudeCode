...
# (they are noisy and carry whatever happened to be on screen when they were
# taken). The default path is the original author's machine, so his runs are
# unchanged; anyone else points AGENT_CAPTURE_DIR at their own copy. When the
# captures are absent the cases skip with a reason rather than failing, because
# their absence says nothing about whether the detector works.

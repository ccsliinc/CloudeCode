...
//   .reset-server-btn    320x40, content box 278x6, for a 19px line
...
// After: 71.7x38, 320x53, 320x45, 21.3x20, and a 28x28 transparent square.
//
...
const OPT_OUTS = [
    ...
    ['styles.css', styleRules, '.reset-server-btn', ['height']],
    ['styles.css', styleRules, '.new-session-btn', ['height']],
...
test('.reset-server-btn and .new-session-btn keep their full-width layout', () => {
    for (const sel of ['.reset-server-btn', '.new-session-btn']) {
        ...
        assert.equal(decl(body, 'width'), '100%',
            `${sel} is a full-width row; height: auto must not have replaced that`);
    }
});

...
The login in that quote is a typo. It means **`ccsliinc`**, which is what the
author field on the issues themselves says, and it is us.
...
Read that second line before you assume it is unfair to us. It is a queue
order, not a budget. Nothing in it caps what we work or reserves anything for
anybody, and our own issues do not stop being ours to work. It says only which
free issue an idle agent reaches for first.

This REPLACES sorting on the priority label alone, which is what this skill
said until today.

Two edge cases, decided by the ruling rather than left to each agent to invent:
...
**AUTHORSHIP SETS ORDER, NEVER OWNERSHIP.** This is the half of the rule an
agent is most likely to drop, so it is stated on its own. Taking your turn in
a queue is not claiming anything. It decides which free issue you reach for
FIRST and nothing beyond that: it reserves no issue, it fences off no file and
no subsystem, it gives the author no veto, and it does not make an issue
theirs to keep. Any agent may still work any free issue. **A draft PR is still
the only thing that makes one taken**, which is the claim mechanism described
under "Claiming" below and it is untouched by this. An author whose issue
somebody else picked up has no claim to it back. Order, not exclusivity.
...
json, and `work.sh free` runs exactly this.
...
**This section is a MIRROR of the other party's, not an independent scheme.**
Theirs is the one Adam ruled on and it lives at `.claude/skills/work/SKILL.md`
on `Adoom666/CloudeCodeDev`, branch `master`, under the same heading, carried
there in commit `08b48a7`; the ruling itself is recorded in their
`docs/DECISIONS.md` under "Free issues are grabbed author first, then
priority". The two are meant to agree word for word on the order, the two edge
cases, the tie-break and the order-not-ownership clause, so that an agent on
either side queues the same list. **If they ever drift, THEIRS is the one to
check**, because the ruling was given on their repo and recorded there first.
Fix this file to match rather than working around the difference, and say so
in the commit.

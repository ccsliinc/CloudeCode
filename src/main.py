...
async def lifespan(app: FastAPI):
    ...
    if auth_cfg.uploads.enabled:
        ...
        upload_sweeper = UploadSweeper(
            ...
            # place projects live. An unreadable datastore yields an
            # empty list here, and that is the safe direction: the
            # sweeper then only sweeps the default upload dir rather
            # than sweeping paths it could not verify.
            project_paths=[
                p["path"]
                for p in resolve_projects(settings.get_state_dir()).projects
            ],

...
async def _apply_session_theme(
    ...
    THIS PINS A THEME TO ONE SESSION, AND IT WRITES ONE STORE.
      ...
      * Validates the tmux name against the known-sessions set (owned
        ...
        an arbitrary key-value endpoint.
    ...
    ``resolve_project_theme`` reads the pin first would be worse than the
    original bug: the folder's default would silently follow whichever
    session was themed last, changing what every never-pinned session in
    it paints. Setting a project default is a separate, deliberate act
    and goes through ``set_project_theme``.
...
async def set_session_theme(
    ...
    """Set (or clear) this session's own pinned theme.

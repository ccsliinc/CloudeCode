...
* NEVER auto-updates. It reports, and offers the command. Pulling code out
  from under a running agent session unattended is not acceptable.
* Which remote: the checkout's own ``origin`` by default, because that is
  what ``upgrade.sh`` actually pulls from - a check against a different
  remote than the upgrade would be answering a question nobody asked. An
  install tracking the public upstream and one tracking a fork are both
  legitimate, so ``updates.remote`` in config.json overrides it with an
  explicit URL.
"""
...
from src.core.version import normalize_tag, repo_root, resolve_version
...
# Where the answer is cached between process restarts.
CACHE_PATH = Path.home() / ".cloude-code" / "update-check.json"
...
def discover_origin_remote(root: Path) -> str:
    ...
    Returns:
        The origin URL, or "" when git is missing or there is no origin.
    """
    try:
...
class UpdateChecker:
    ...
    def __init__(
        ...
        upgrade_command: str = "~/.cloude-code/bin/upgrade.sh",
    ) -> None:
        """Create a checker.

...
5. A LOOSE ``git describe --tags --always``, for example
   "0.8.1-3-gabc1234". Last, deliberately: it is the most honest answer but
   the ugliest, and it is not a release tag, so the self check will correctly
   report "this install is not at a release tag" when it is what resolved.
...
def _git_describe(root: Path | None, args: list[str]) -> str:
    ...
    Returns:
        The described tag, or "" when git is missing, the directory is not a
        work tree, the call timed out, or nothing was described.
    """
...
def describe_git_tag(root: Path | None = None) -> str:
    ...
    """
    return _git_describe(root, ["--tags", "--always"])

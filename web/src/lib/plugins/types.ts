 ...
 *
 * Description: PURE DATA PLUS ONE EFFECT. Everything about how the
 *   control looks is a function of the row, so one renderer
 *   (`session-card-action-html.ts`) draws every contribution the same
 *   way and a future Svelte card can draw the same values as elements
 *   instead of as a string. `run` is the only thing that touches the
 *   world.
 *
 *   THE FUNCTIONS TAKE THE ROW BECAUSE THESE CONTROLS ARE STATEFUL. The
 *   mark-unread control's label, glyph, class and pressed state all flip
 *   with `row.unread`; a static label field would have forced the
 *   renderer to special-case it immediately.
 * Example:
 *   const a: SessionCardAction = {
 *       label: (r) => (r.unread ? 'clear unread flag' : 'mark unread'),
 *       icon: () => '<svg .../>',
 *       className: () => 'mark-unread-toggle',
 *       attrs: (r) => ({ 'data-mark-unread': r.name }),
 *       run: async (r, ctx) => { await patch(r); await ctx.refresh(); },
 *   };
 */
export interface SessionCardAction {
    /** Visible label, the `title` and the `aria-label`. Never empty. */
    label(row: SessionCardRow): string;
    /** The glyph, as a self-contained `<svg>` string. */
    icon(row: SessionCardRow): string;
    /** The control's CSS class list. The stylesheets key on this. */
    className(row: SessionCardRow): string;
    /**
     * Extra attributes, escaped by the renderer. Attribute ORDER in the
     * emitted string is the renderer's, not this map's: nothing visible,
     * assistive or behavioural depends on it.
     */

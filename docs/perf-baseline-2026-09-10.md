...
**The known gap is CLOSED, and the cause was not what this note used to say.** This document previously recorded `settings open, cold/warm` as `n/a` in every column and attributed it to `#settingsBtn` reporting "not visible" to a click after the preceding menu interaction, with a `wait_for_selector(state="visible")` added and not fully resolving it. That reads as a timing problem and it was not one. Diagnosed 2026-09-10: **`#settingsBtn` is not in the header row at all.** `client/js/header-menu.js` re-parents it, with `logoutBtn`, into `#header-menu-panel` in `_fold()`; `applyLayout()` calls `_fold()` unconditionally at EVERY width (its own module docstring: "an overflow, not a responsive fold any more"); and `.header-menu-panel` is `display: none` until the kebab is opened. So the button genuinely has no box until somebody opens the overflow menu, for Playwright and for a human alike. No wait could help, and `force=True` could not either, because force bypasses hit-testability and not "has no box at all". That is also why it failed in EVERY run rather than intermittently. **The application is not at fault and nothing is filed against it**: the harness step was measuring an interaction nobody can perform. `run_baseline.py` now opens `#header-menu-toggle` first, OUTSIDE the timed callback so the menu's own animation is not billed to "settings open", and no forced click was introduced. Two further facts came out of measuring rather than reading, and both are in the code as comments: the open has to be IDEMPOTENT, because the toggle toggles and a control click inside the panel collapses it, so an unconditional click can just as easily shut the menu; and the close has to WAIT for `#settings-panel-body` to actually be hidden, because the modal overlay sits over the header and a re-open while it is still on screen clicks the overlay instead of the toggle. Measured after the fix, N=1 `--quick`, on a box at load average 19 to 28, and repeated because the original failure happened in EVERY run so one success proves nothing: run A gave `settings open, cold` 38.6 ms and `warm` 157.1 ms, run B gave 81.4 ms and 41.9 ms. Both cells reported a real number with a non-zero sample count in both runs, against `n/a / n=0` for both before, and neither run logged a `settings_open_*` step failure. THE SPREAD BETWEEN THE TWO RUNS IS THE LOAD, NOT THE FEATURE, which is the whole reason these are quoted as proof that the step measures and NOT as a baseline. The general lesson is in `perf_browser.click_tolerant`'s docstring now: that helper turning "I could not click that" into a silent pass is what let a WRONG SELECTOR masquerade as a slow one for a whole baseline, so when a step reports no measurement at all, suspect the selector before the clock.

**THE TABLE BELOW IS STILL THE OLD CAPTURE, AND ITS TWO `settings open` ROWS ARE STALE ON PURPOSE.** They are left reading `n/a` because this document is a dated record of one run, and the run really did produce `n/a`; overwriting two cells with numbers from a different, single-N run would make the row lie about which capture it came from. **A FRESH 1/10/50 BASELINE IS OWED AND HAS NOT BEEN TAKEN.** The machine-load note above already says these absolute figures need re-measuring on an idle box before they judge a later phase, and the box has been at load average 12 to 28 continuously through the work that closed this gap, so taking the re-capture now would reproduce exactly the defect that note warns about. Take it on a quiet machine, without `--quick`, and write it to a new dated file rather than editing this one.
